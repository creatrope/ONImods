...
def clone_project(source_dir, target_dir):
    ...
    # Patterns to replace
    patterns = [
        # Namespace and Debug.Log brackets
        ...
        # If someone has manually embedded module names like [SmartSweep4] anywhere else
        ...
        # YAML files (mod.yaml, mod_info.yaml)
    ...
    for root, dirs, files in os.walk(target_dir):
        for filename in files:
            ...
            # Update text content inside files
            if filepath.endswith(('.cs', '.csproj', '.sln', '.txt', '.yaml', '.yml')):
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python clone.py <source_directory> <target_directory>")
        sys.exit(1)

    source_directory = sys.argv[1]
    target_directory = sys.argv[2]
    clone_project(source_directory, target_directory)
